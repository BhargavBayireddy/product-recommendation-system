...
# ----- Aggressive collaborative row (global feed) -----
def collaborative_candidates_aggressive(uid, top_k=12):
    ...
    Rank by my model score + tiny recency boost. Exclude my liked/bagged items.
    """
    ...
    # users who also liked something I liked (not me)
    similar_uids = {
    ...
    # recency boost from global feed
    latest_ts = {}
...
def recommend(uid, k=48):
    ...
    # collaborative row (FIRST priority)
...
def card_row(df: pd.DataFrame, section_key: str, title: str, subtitle: str = "", show_cheese: bool=False, allow_remove=False):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        with col:
            ...
            if allow_remove:
                # choose action default = like (safe)
...
def page_home():
    ...
    card_row(because.head(12), "because", "🎧 Because you liked…",
             "More of what matched your vibe 😎", True)
